...
double kp_roll = 10.0;
double ki_roll = 0.05;   // 0.04;
double kd_roll = 0.0;  // 18.0

//double kp_roll = 0.0;    // 10.0
//double ki_roll = 0.0;
//double kd_roll = 10.0;
...
float desired_angle = 0.0;        //This is the angle in which we whant the drone to stay steady
...
//  mySoftwareSerial.println("Starting");
  ...
  DDRB = B11110111;   //Setting PORTB, digital pin 8,9 1nd 10 to OUTPUT and digital pin 11 to INPUT
  DDRD = B11100101;   //Setting PORTD, digital pin 7,6 and 5 to OUTPUT and digital pin 1,3 and 4 to INPUT
...
void loop() {
  ...
  get_mpu_6050_data(); 
  pitch = Total_angle[0] + 5.5;   // 5.5 is gyro pitch angle error
  roll = Total_angle[1] + 2.5;    // 5.5 is gyro roll angle error
  
  pid_controller();
  
  if((-3 <= pitch) && (pitch <= 3 ) && (-3 <= roll) && (roll <= 3) && !isDroneLevel){
    isDroneLevel = true; 
//    mySoftwareSerial.println("Level *************");
  }else{
    isDroneLevel = false; 
  }
...
//    Serial.println("Dropping Ball");
  ...
  if((rc_values[RC_CH3] <= 1000) && (rc_values[RC_CH3] != 0) && (rc_values[RC_CH4] <= 1000) && (rc_values[RC_CH4] != 0) && !isArmed && isDroneLevel){
    isArmed = true;
//    Serial.println("ARMED");
    PORTD |= B10000000;
    PORTB |= B00000001;
  }
  if((rc_values[RC_CH3] <= 1000) && (rc_values[RC_CH3] != 0) && (rc_values[RC_CH4] >= 2000) && (rc_values[RC_CH4] != 0) && isArmed){
    isArmed = false;
...
//    throttle4 = rc_values[RC_CH3];
    ...
    throttle1 = 170 + throttle1 - (int)pid_roll;
    throttle4 = 170 + throttle4 - (int)pid_roll;
    throttle2 = 170 + throttle2 + (int)pid_roll;
    throttle3 = 170 + throttle3 + (int)pid_roll;
  ...
  */
    ...
    */
    ...
    if(throttle1 < 1170) throttle1= 1170;
    if(throttle2 < 1170) throttle2= 1170;
    if(throttle3 < 1170) throttle3= 1170;
    if(throttle4 < 1170) throttle4= 1170;
...
//  mySoftwareSerial.print("pitch : ");
//  mySoftwareSerial.println(pitch);
//  mySoftwareSerial.print("\t");
//  mySoftwareSerial.println("Total_angle[0] : ");
//  mySoftwareSerial.println(Total_angle[0]);
//  mySoftwareSerial.print("\t");
//  mySoftwareSerial.print("Total_angle[1] : ");
//  mySoftwareSerial.print(Total_angle[1]);
//  mySoftwareSerial.print("\t");
//  mySoftwareSerial.print("roll : ");
//  mySoftwareSerial.println(roll);

  
//  mySoftwareSerial.print("\t");
//  mySoftwareSerial.print("error_roll: ");
//  mySoftwareSerial.print(error_roll);
//  mySoftwareSerial.print("\t");
//  mySoftwareSerial.print("error_pitch: ");
//  mySoftwareSerial.println(error_pitch);
//  mySoftwareSerial.print("\t");
//  mySoftwareSerial.print(pid_roll);
//  mySoftwareSerial.print("\t");
//  mySoftwareSerial.print(pid_pitch);
//  mySoftwareSerial.print("\t");
//  throttle1 = rc_values[RC_CH3];
//  mySoftwareSerial.println(throttle1);
